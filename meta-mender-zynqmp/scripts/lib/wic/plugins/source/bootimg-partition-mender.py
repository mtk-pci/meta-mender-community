...
# files listed in IMAGE_BOOT_FILES bitbake variable, and including 
# the same structured files in rootfs/uboot/ for Mender script updates.
#
# AUTHORS
# Matt Keener <keener (at] patterncomputer.com> adapted from
# Maciej Borzecki <maciej.borzecki (at] open-rnd.pl>
#
# Yocto version Kirkstone (4.0.10):
# https://docs.yoctoproject.org/4.0.10/dev-manual/common-tasks.html#creating-partitioned-images-using-wic
#
...
class BootimgPartitionPlugin(SourcePlugin):
    ...
    listed in IMAGE_BOOT_FILES bitbake variable, and 
    including the same structured files in rootfs/uboot/ 
    for Mender script updates.
    """

    name = 'bootimg-partition-mender'
    ...
    def do_prepare_partition(cls, part, source_params, cr, cr_workdir,
        ...
        - copies all files listed in IMAGE_BOOT_FILES variable to both the
          boot partition and the /uboot folder in rootfs for Mender updates
        """
        hdddir = "%s/boot.%d" % (cr_workdir, part.lineno)
        ubdir = "%s/uboot" % (rootfs_dir)
